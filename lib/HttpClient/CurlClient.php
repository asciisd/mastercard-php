...
//        $params = Util\Util::objectsToIds($params);
        ...
        if (($method == 'post') && (Mastercard::$maxNetworkRetries > 0)) {
        ...
        if (!Mastercard::getVerifySslCerts()) {
    ...
    private function handleCurlError($url, $errno, $message, $numRetries)
    {
        switch ($errno) {
            ...
            case CURLE_OPERATION_TIMEOUTED:
                $msg = "Could not connect to Mastercard ($url).  Please check your "
                    . "internet connection and try again.  If this problem persists, "
                    . "you should check Mastercard's service status at "
                    . "https://twitter.com/mastercardstatus, or";
                break;
            case CURLE_SSL_CACERT:
            case CURLE_SSL_PEER_CERTIFICATE:
                $msg = "Could not verify Mastercard's SSL certificate.  Please make sure "
                    . "that your network is not intercepting certificates.  "
                    . "(Try going to $url in your browser.)  "
                    . "If this problem persists,";
                break;
            default:
                $msg = "Unexpected error communicating with Mastercard.  "
                    . "If this problem persists,";
        }
        $msg .= " let us know at support@mastercard.com.";
    ...
    {
        if ($numRetries >= Mastercard::getMaxNetworkRetries()) {
            return false;
        }
        ...
        if (isset($rheaders['mastercard-should-retry'])) {
            if ($rheaders['mastercard-should-retry'] === 'false') {
                return false;
            }
            if ($rheaders['mastercard-should-retry'] === 'true') {
                return true;
            }
        }
        ...
        //
        // Note that we expect the mastercard-should-retry header to be false
    ...
    private function sleepTime($numRetries, $rheaders)
    {
        ...
        $sleepSeconds = min(
            Mastercard::getInitialNetworkRetryDelay() * 1.0 * pow(2, $numRetries - 1),
            Mastercard::getMaxNetworkRetryDelay()
        );
        ...
        $sleepSeconds = max(Mastercard::getInitialNetworkRetryDelay(), $sleepSeconds);

        // And never sleep less than the time the API asks us to wait, assuming it's a reasonable ask.
        $retryAfter = isset($rheaders['retry-after']) ? floatval($rheaders['retry-after']) : 0.0;
        if (floor($retryAfter) == $retryAfter && $retryAfter <= Mastercard::getMaxRetryAfter()) {
    ...
    private function canSafelyUseHttp2()
    {
        ...
        // (cf. https://github.com/curl/curl/issues/2416), which Mastercard use.
