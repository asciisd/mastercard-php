...
class ApiRequestor
{
    ...
    /**
     * Creates a telemetry json blob for use in 'X-Mastercard-Client-Telemetry' headers
     * @static
     *
    ...
    private static function _telemetryJson($requestTelemetry)
    {
        ...
        if ($result != false) {
            ...
            Mastercard::getLogger()->error("Serializing telemetry payload failed!");
            return "{}";
        }
    }
    ...
    public function __construct($apiKey = null, $apiBase = null)
    {
        $this->_apiKey = $apiKey;
        if (!$apiBase) {
            $apiBase = Mastercard::getGatewayUrl();
        }
    ...
    {
        $uaString = 'Mastercard/ PhpBindings/' . Mastercard::VERSION;

        $langVersion = phpversion();
        $uname = php_uname();

        $appInfo = Mastercard::getAppInfo();
        $ua = [
            'bindings_version' => Mastercard::VERSION,
            'lang' => 'php',
        ...
        $merchantId = Mastercard::getMerchantId();
        $password = $apiKey;

        $defaultHeaders = [
            'X-Mastercard-Client-User-Agent' => json_encode($ua),
            'User-Agent' => $uaString,
    ...
    private function _requestRaw($method, $url, $params, $headers)
    {
        ...
        if (!$myApiKey) {
            $myApiKey = Mastercard::$apiKey;
        }

        if (!$myApiKey) {
            $msg = 'No API key provided.  (HINT: set your API key using '
                . '"Mastercard::setApiKey(<API-KEY>)".  You can generate API keys from '
                . 'the Mastercard web interface.  See https://mastercard.com/api for '
                . 'details, or email support@mastercard.com if you have any questions.';
            throw new Exception\AuthenticationException($msg);
        }

        // Clients can supply arbitrary additional keys to be included in the
        // X-Mastercard-Client-User-Agent header via the optional getUserAgentInfo()
        // method
        $clientUAInfo = null;
        ...
        if (Mastercard::$apiVersion) {
            $defaultHeaders['Mastercard-Version'] = Mastercard::$apiVersion;
        }
        ...
        if (Mastercard::$enableTelemetry && self::$requestTelemetry != null) {
            $defaultHeaders["X-Mastercard-Client-Telemetry"] = self::_telemetryJson(self::$requestTelemetry);
        }
