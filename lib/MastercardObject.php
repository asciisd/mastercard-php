...
/**
 * Class MastercardObject
 *
 * @property string $id
 *
 * @package Mastercard
 */
class MastercardObject implements \ArrayAccess, \Countable, \JsonSerializable
{
    protected $_opts;
    ...
    public function __set($k, $v)
    {
        ...
        $this->_values[$k] = Util\Util::convertToMastercardObject($v, $this->_opts);
    ...
    public function &__get($k)
    {
        ...
        if (!empty($this->_values) && array_key_exists($k, $this->_values)) {
            ...
            $message = "Mastercard Notice: Undefined property of $class instance: $k. "
                . "HINT: The $k attribute was set in the past, however. "
                . "It was then wiped when refreshing the object "
                . "with the result returned by Mastercard's API, "
                . "probably as a result of a save(). The attributes currently "
                . "available on this object are: $attrs";
            Mastercard::getLogger()->error($message);
            return $nullval;
        } else {
            $class = get_class($this);
            Mastercard::getLogger()->error("Mastercard Notice: Undefined property of $class instance: $k");
            return $nullval;
        }
    }
    ...
    public function refreshFrom($values, $opts, $partial = false)
    {
        ...
        if ($values instanceof MastercardObject) {
    ...
    {
        foreach ($values as $k => $v) {
            // Special-case metadata to always be cast as a MastercardObject
            // This is necessary in case metadata is empty, as PHP arrays do
            // not differentiate between lists and hashes, and we consider
            // empty arrays to be lists.
            if (($k === "metadata") && (is_array($v))) {
                $this->_values[$k] = MastercardObject::constructFrom($v, $opts);
            } else {
                $this->_values[$k] = Util\Util::convertToMastercardObject($v, $opts);
            }
            if ($dirty) {
                $this->dirtyValue($this->_values[$k]);
            }
    ...
    public function serializeParameters($force = false)
    {
        ...
        foreach ($this->_values as $k => $v) {
            ...
            //   3. Its value is a MastercardObject. A MastercardObject may contain modified
            //      values within in that its parent MastercardObject doesn't know about.
            //
            $original = array_key_exists($k, $this->_originalValues) ? $this->_originalValues[$k] : null;
            $unsaved = $this->_unsavedValues->includes($k);
            if ($force || $unsaved || $v instanceof MastercardObject) {
    ...
    public function serializeParamsValue($value, $original, $unsaved, $force, $key = null)
    {
        ...
        if ($value === null) {
            ...
            if (Util\Util::isList($value)) {
                ...
                return Util\Util::convertToMastercardObject($value, $this->_opts)->serializeParameters();
            }
        } elseif ($value instanceof MastercardObject) {
    ...
    /**
     * Returns a pretty JSON representation of the Mastercard object.
     *
     * @return string The JSON representation of the Mastercard object.
     */
    ...
    /**
     * Sets all keys within the MastercardObject as unsaved so that they will be
     * included with an update when `serializeParameters` is called. This
     * method is also recursive, so any MastercardObjects contained as values or
    ...
    protected static function deepCopy($obj)
    {
        if (is_array($obj)) {
            ...
            return $copy;
        } elseif ($obj instanceof MastercardObject) {
    ...
    /**
     * @return object The last response from the Mastercard API
     */
    ...
    /**
     * Sets the last response from the Mastercard API
     *
